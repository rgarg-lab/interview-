...
router.post('/generate-feedback', async (req, res) => {
  ...
  for (let i = 0; i < audio_questions.length; i++) {
    const answer = audio_answers[i]?.trim();
     if (!answer) {
    evaluations.push({
      score: "0/10",
...
b) Good answer but could be more detailed.`;
    ...
    const scoreLine = raw.split('\n').find((l) => l.startsWith('a)')) || 'a) 0/10';
const feedbackLine = raw.split('\n').find((l) => l.startsWith('b)')) || 'b) No response detected.';

const score = scoreLine.replace(/^a\)\s*/, '');
const feedback = feedbackLine.replace(/^b\)\s*/, '');
